 ...
 */
public class BBPrioLimitedGraphFactory extends GraphAlgorithmFactory {
	...
	public Algorithm createAlgorithm() {
		return new BBPrioLimitedGraph(graph, prioDings);
	}
	...
	public RequestData readRequestData(ObjectMapper mapper, Responder responder, FullHttpRequest request) throws IOException {
		...
		if (content.readableBytes() > 0) {
			...
			int hintLevel = 0;
			int nodeCount = 0;
			BBPrioLimitedRequestData.LevelMode mode = BBPrioLimitedRequestData.LevelMode.AUTO;
			int lat = 0, lon = 0;
				...
				if (token == JsonToken.FIELD_NAME) {
					...
					if ("bbox".equals(fieldname)) {
						...
						if (m.equalsIgnoreCase("hinted")){
							mode = BBPrioLimitedRequestData.LevelMode.HINTED;
						} else if (m.equalsIgnoreCase("exact")) {
							mode = BBPrioLimitedRequestData.LevelMode.EXACT;
						}
			...
			return new BBPrioLimitedRequestData(this.getURLSuffix(), bbox, mode, minLen, maxLen, maxRatio, nodeCount, hintLevel);
		} else {
